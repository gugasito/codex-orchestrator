...
cannot be changed by a skill. If the session is using neither Sol nor Astra,
state the mismatch and ask the user to select one of those models before
claiming this workflow is active.
...
acceptance criteria need independent evidence. The controller never runs the checks.
